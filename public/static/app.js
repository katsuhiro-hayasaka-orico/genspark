...
async function renderProject() {
  ...
  <div class="panel new-project-scatter"><h4>進捗率 × コスト消化率</h4><div style="height:300px"><canvas id="np_scatter"></canvas></div><div id="np_scatter_meta" class="muted" style="margin-top:6px"></div><div id="np_scatter_notice" class="muted" style="margin-top:6px"></div><p class="muted">左上は要確認（進捗低・消化高）/ 右下は進捗先行の可能性。</p></div></div>
  ...
  const renderAll = () => {
    ...
    const scatterData=top.filter(r=>Number.isFinite(Number(r.progressRate))&&Number.isFinite(Number(r.costConsumptionRate))).map(r=>({x:Number(r.progressRate),y:Number(r.costConsumptionRate),r:5}));
    const excludedCount = top.length - scatterData.length;
    ...
    document.getElementById('np_scatter_notice').textContent = scatterData.length === 0 ? '進捗率またはコスト消化率を算出できるデータがありません。マスタCSVの進捗状況、月次金額CSVの予算金額・見込金額・対象年月を確認してください。' : '';
