...
## CSVフォーマット（指定レイアウト）
### budget_master.csv
- 主要列: `period`, `management_no`, `item_no`, `budget_category`, `expense_item_code`, `system_code`, `expense_item_name`, `system_name`, `system_classification_name`, `monthly_amount`

### budget_detail.csv
- 主要列: `management_no`, `item_no`, `expense_item_code`, `system_code`, `fiscal_period`, `target_year_month`, `value_type`, `amount`
- `value_type` は `plan` / `forecast` / `actual` を想定（内部で横持ちへ変換）
