...
| `state.ui.trendAggregationUnit` | 推移表示単位（月別/四半期別/期別/累計推移） |
...
表示単位（月別/四半期別/期別/累計推移）、表示件数、指標選択、金額単位切替、推移グラフ、変動ランキングのカードで構成します。
...
表示単位、表示件数、指標、金額単位を変更すると再描画します。

## 8. フィルター・ソート仕様

グローバルフィルターの対象期間でデータを絞り込み、画面内の表示単位で月別/四半期別/期別/累計推移に分解します。ランキングは変動額/率に基づきます。

## 9. グラフ・テーブル仕様

`buildTimeSeries` が `state.ui.trendAggregationUnit` に応じて時系列バケットを作成し、Chart.js の折れ線/棒グラフを描画します。
