...
KPIストリップ、金額単位切替、予算vs見込み/実績グラフ、報告用サマリー、差額ランキングで構成されます。期全体カードおよび上部KPIカードと重複する選択期間カードは表示しません。
...
| 報告用サマリーの期表示 | 第67期は2026年4月〜2027年3月のため `第67期（FY2027）` と表示。その他の期も期末年をFYとして表示 |
...
グローバルフィルター（対象期間、部署、分析視点等）を利用します。対象期間はコンパクトなトリガーからポップオーバーを開き、当月・前月・当期通期・前期通期・直近3か月・直近12か月・データ最新月・カスタム範囲を選択します。カスタム範囲では対象月または対象期の開始/終了を指定します。対象期は67期=2026年4月〜2027年3月、66期=2025年4月〜2026年3月として月範囲へ展開し、選択範囲の月次データだけを累計します。報告用サマリーの期セレクトはこの月範囲の期末年をFYとして表示するため、第67期は `FY2027` です。差額ランキングは差額絶対値または差額の大きい順で表示される実装です。
...
全体サマリー画面では、Report exportエリアの「PDF出力」右側に「AI分析用プロンプト」ボタンを表示します。ボタン押下後、現在のグローバルフィルター（対象期間、部門、分類軸、対象）を含めて `POST /api/ai-prompt` を呼び出し、主要KPI、月次推移、差額上位、アラート、データ品質注意を含むMarkdownプロンプトを生成できます。既定では明細全件は含めません。担当者名はユーザーが明示選択した場合のみ含め、契約番号は含めるデータ対象外です。
