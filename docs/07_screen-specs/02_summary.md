...
KPIストリップ、金額単位切替、選択期間カード、予算vs見込み/実績グラフ、報告用サマリー、差額ランキングで構成されます。期全体カードは表示しません。
...
| 選択期間カード | グローバルフィルターで選択した対象期間の `totalPlan`, `comparable`, `variance` |
...
全体サマリー画面では、現在のグローバルフィルター（対象期間、部門、分類軸、対象）を含めて `POST /api/ai-prompt` を呼び出し、主要KPI、月次推移、差額上位、アラート、データ品質注意を含むMarkdownプロンプトを生成できます。既定では明細全件は含めません。担当者名はユーザーが明示選択した場合のみ含め、契約番号は含めるデータ対象外です。
