...
| `fiscal_period` | 期 | 会計期フィルター・集計 | 全体、推移、カテゴリ、減価償却 | CSV `期`、または年月から導出 | 一部はい | 年月からの導出は `deriveFiscalPeriodFromYearMonth` |
| `target_year_month` | 対象年月 | 月次集計、推移、スコープ | 全体、推移、明細、新規案件 | CSV年月列、正規化処理 | いいえ | `YYYYMM` 形式 |
