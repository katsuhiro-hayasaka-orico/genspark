...
| 全体サマリーのReport exportエリアに配置する「AI分析用プロンプト」ボタン（表示対象は `AI_PROMPT_VISIBLE_PAGES`）、右ドロワー、コピー、Copilot起動、Markdown/JSON保存 | `public/static/app.js` の `AI_PROMPT_VISIBLE_PAGES` / `shouldShowAiPromptAction` / `renderExportControls` / `updateAiPromptTopbarButton` / `openAiPromptDrawer` / `generateAiPrompt` / `copyAiPrompt` / `saveAiPromptMarkdown` / `saveAiPromptJson` |
