...
- ダッシュボード系画面の上部操作に「AI分析用プロンプト」ボタンを表示します。グローバルフィルターがある画面ではフィルター行にも同ボタンを表示します。
- ボタン押下で右ドロワーを開き、対象画面・対象期間・部門・対象条件、含めるデータ、機密項目マスク設定を確認できます。
- 「プロンプト生成」は `POST /api/ai-prompt` を呼び出し、生成Markdownをプレビュー欄に表示します。
