...
- メインCSVの分析画面（全体サマリー、推移分析、カテゴリ別分析、アラート、ベンダー／契約更新、明細ドリルダウン）のトップバー操作に「AI分析用プロンプト」ボタンを表示します。追加CSV画面（新規案件コスト、減価償却シミュレーション、OACIS実績）では、追加CSV専用プロンプトへの将来カスタマイズを想定し、現時点では見た目上非表示にします。メインCSV上部共通のグローバルフィルター行や出力操作欄にも表示しません。
