...
| 適用範囲 | `--app-zoom` を通じて `.layout`（アプリ本体）に限定して適用し、body直下のドロワー/モーダル類は倍率対象外 |
| 操作 | トップバーの `−`, `＋`, `100%`、キーボードショートカット対応あり |

## 10. レスポンシブ対応

`style.css` にはモバイルメニュー、サイドバー開閉、グリッドのレスポンシブ調整が定義されています。詳細ブレークポイントはCSSを参照してください。
サイドバーはビューポート高いっぱいに固定し、外側ではなく `#sidebarNav` のみを縦スクロール領域にします。これにより表示倍率を変更した場合も、通常/折りたたみ/モバイルメニュー状態でメニュー下部が画面途中で切れないようにします。
...
- 全体サマリー画面のみ、Report exportエリアの「PDF出力」右側に「AI分析用プロンプト」ボタンを表示します。グローバルフィルターエリア、アプリ共通トップバー、他画面の出力操作欄には表示しません。追加CSV画面（新規案件コスト、減価償却シミュレーション、OACIS実績）用プロンプトは将来カスタマイズを想定します。
- ボタン押下で右ドロワーを開き、対象画面・対象期間・部門・対象条件、含めるデータを確認できます。担当者名はチェック時のみ含め、契約番号と機密項目マスク切替は表示しません。
- 右ドロワーと背景オーバーレイはアプリ本体の表示倍率対象外としてビューポートに固定し、ドロワー表示中は背面スクロールを抑止します。内容が多い場合はドロワー本文のみをスクロールします。背景クリックまたはEscapeキーで閉じます。
