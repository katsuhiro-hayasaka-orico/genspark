...
| 共通 | 表示設定 | しきい値、KPI並び順、表示倍率、テーマを調整する |
...
| フィルター | 操作と結果 |
|---|---|
| 集計軸 | 「月次」「四半期」「通期」を選択します。集計の単位が切り替わります。 |
| 対象期(自) / 対象期(至) | 期間の開始と終了を選択します。対象期間が絞り込まれます。 |
| 対象月 | 特定の年月を選択します。対象月に絞り込まれます。 |
| 部門 | 「全部門」または特定部門を選択します。部門別に絞り込まれます。 |
| 観点 | 「費目」「システム」「固定・変動」「投資・運用」を選択します。分析の見方が変わります。 |
| 対象 | 「すべて」「継続案件」「新規案件」やベンダー別の対象を選択します。 |

フィルターを変更すると、表示中の画面が自動的に更新されます。
...
4. 表示倍率をスライダーまたは数値入力で変更します。
5. 表示テーマを選択します。
6. 「反映」をクリックします。
7. 「表示設定を反映しました」と表示されます。
