...
全体サマリー、推移分析、カテゴリ別分析、アラート、ベンダー／契約更新、明細ドリルダウンでは、画面上部操作から「AI分析用プロンプト」ドロワーを開けます。追加CSV画面（新規案件コスト、減価償却シミュレーション、OACIS実績）では、追加CSV専用プロンプトへの将来カスタマイズを想定し、現時点ではトップバーの「AI分析用プロンプト」ボタンを見た目上非表示にします。
