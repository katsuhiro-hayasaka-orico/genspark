...
全体サマリー、推移分析、カテゴリ別分析、アラート、ベンダー／契約更新、明細ドリルダウン、新規案件コスト、減価償却シミュレーション、OACIS実績では、画面上部操作から「AI分析用プロンプト」ドロワーを開けます。MVPの主対象は `summary` / `alert` / `detail` ですが、未対応画面でも共通KPI中心の汎用プロンプト生成を試行します。
