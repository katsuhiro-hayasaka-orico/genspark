...
| 共通・ユーティリティ | `settings` | 表示設定 | テーマ、表示倍率、しきい値、KPI順序、対象期間ショートカット、分類軸管理 | `renderSettings` | なし（localStorage/state中心） | [11_settings.md](./07_screen-specs/11_settings.md) | クイック設定から遷移 |
...
全体サマリーでは、Report exportエリアの「PDF出力」右側に配置した「AI分析用プロンプト」ボタンからドロワーを開けます。グローバルフィルターエリアやアプリ共通トップバーには配置しません。他画面（推移分析、カテゴリ別分析、アラート、ベンダー／契約更新、明細ドリルダウン、新規案件コスト、減価償却シミュレーション、OACIS実績）では見た目上非表示とします。追加CSV画面用プロンプトは将来カスタマイズを想定します。
